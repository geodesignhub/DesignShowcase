...
def api_root():
    ...
    if projectid and cteamid and apitoken and synthesisid:
        ...
        myAPIHelper = GeodesignHub.GeodesignHubClient(
            url=config.apisettings["serviceurl"], project_id=projectid, token=apitoken
        )
        # Download Data
        ...
        for f in finalsynthesis["features"]:
            ...
            myBagofWordsGenerator.addtoCorpusDict(
                f["properties"]["diagramid"], f["properties"]["description"]
            )
        ...
        if ss:
            ...
            result = q.enqueue(
                utils.createSenteceSimilarity, {"data": tmpCorpusDict, "key": key}
            )
            sentenceSimilarity = {}
        # sentenceSimilarity ={}
        try:
        ...
        designdata = {
            "systems": systems,
            "synthesis": formattedfinalsynthesis,
            "bounds": bounds,
            "wordfreq": wordfreq,
            "fuzzymatches": sentenceSimilarity,
            "diagrams": diagrams,
        }
        msg = {
            "status": 1,
